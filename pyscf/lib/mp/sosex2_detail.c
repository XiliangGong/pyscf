...
void elementwise_multiply(int size, const double *array1, const double *array2, double *result) {
    ...
    //     __m256d vec1 = _mm256_loadu_pd(&array1[i]);  // 加载 4 个 double 数据
    //     __m256d vec2 = _mm256_loadu_pd(&array2[i]);
    //     __m256d res = _mm256_mul_pd(vec1, vec2);     // SIMD 逐元素乘法
    //     _mm256_storeu_pd(&result[i], res);          // 存储结果
    // }
    ...
    /*
   //Try to use dgemm, but didn't work because of segment default
    double *diagA = calloc(size * size, sizeof(double));
    double *diagB = calloc(size * size, sizeof(double));
    double *diagR = calloc(size * size, sizeof(double));
    double alpha = 1.0, beta = 0.0;
    // 构造对角矩阵
    for (int i = 0; i < size; ++i) {
        diagA[i * size + i] = array1[i];
        diagB[i * size + i] = array2[i];
    }

    // 使用 dgemm_ 进行矩阵乘法
    dgemm_("N", "N", &size, &size, &size, &alpha, diagA, &size, diagB, &size, &beta, diagR, &size);

    // 提取对角线
    for (int i = 0; i < size; ++i) {
        result[i] = diagR[i * size + i];
    }
...
void energy_sosex(double *ed_out, double *ex_out, const double *u2, const double *iJ, const double *jJ, 
    ...
    printf("Total accumulate time: %.6f seconds\n", total_accumulate_time);
...
/*
    ...
    for tw in tau_cd:
        print(tw.shape) #(4, 53)
        print(u2.shape) #(4, 53, 141)
        u2w = tw[:,:,None] * u2
        vw = lib.einsum('jbB,jbA->BA', u2w, J)
        # u2w (4*53, 141-nauxvB) J (4*53, nauxvA)
        # direct C(nauxB, nauxA) = u2w (4*53, 141-nauxvB) * J(4*53, nauxvA)
        # C-order C(nauxB, nauxA) = J(4*53, nauxvA) * u2w (4*53, nauxvB) 
        # F-order C(A,B) = u2w(A,～) * J（B，～）
        # F-order C(A,B) = trans(u2w)(~,A) * J(B,~)
        # If I want C(B,A), change the site of A and B

        u2new -= 2 * lib.einsum('iaB,BA->iaA', u2w, vw)
        # u2w(4*53-ia, B) * vw(B, A) case 1
        # C-order C(ia,A) = u2w(ia,B) * vw(B,A)
        # C-order C(A,ia) = vw^T(A,B) * u2w^T(B,ia)
        # F-order C(ia,A) = (u2w^T(B,ia))^T(ia,B) * (vw^T(A,B))^T (B,A)
        

        u2w = vw = None
*/
// void update_u2(const double *u2, const double *J, const double *tau_cd, 
//                   int nocc, int nvir, int naux, int nauxv){
...
//     double *u2new_sum = malloc(nov * nauxv * sizeof(double));

//     const double D0 = 0;
//     const double D1 = 1;

//     double *expanded_tau_cd = malloc(naux * nov * sizeof(double));
//     for (int w = 0; w < naux; ++w) {
//         for (size_t idx = 0; idx < nov; ++idx) {
//             expanded_tau_cd[w * nov + idx] = tau_cd[w * nov + idx];
//         }
//     } 

//     for (int w = 0; w < naux; ++w) {

//         // Initialize the u2new as 0
//         for (size_t i =0; i< nov* nauxv; ++i){
//             u2new[i] = J[i];
//         }
//         // vdMul(nvxv_int, u2_i, (double *)expanded_tau_cd[w][i], u2w_i[w]  );

//         // Precompute expanded_tau_cd

//         elementwise_multiply(nvxv, u2, (double *)expanded_tau_cd, u2w);

//         int nov_int = (int)nov;      // 将 size_t 转为 int
//         int nauxv_int = (int)nauxv;  // 同上
//         int naux_int = (int)naux;
//         dgemm_("T", "N", nauxv, nauxv, nov, &D1, J, nov, u2w, nov, &D0, vw, nauxv);

//         dgemm_("T", "T", nov, nauxv, nauxv, &D1, u2w, nov, vw, nauxv, &D0, u2new, nauxv);
...
void update_u2(const double *u2, const double *J, const double *tau_cd, 
    ...
    printf("Entering update_u2 function\n");
    printf("nocc = %d, nvir = %d, naux = %d, nauxv = %d\n", nocc, nvir, naux, nauxv);
    printf("u2 pointer: %p, J pointer: %p, tau_cd pointer: %p\n", u2, J, tau_cd);

    // 示例打印数组的第一个值
    printf("First value of u2: %f\n", u2[0]);
    printf("First value of J: %f\n", J[0]);
    printf("First value of tau_cd: %f\n", tau_cd[0]);
    ...
    for (int w = 0; w < naux; ++w) {
        for (int i = 0; i < nov; ++i) {
                for (int k = 0; k < nauxv; ++k) {
                    ...
                    // printf("tau_cd[%d][%d][%d] = %f\n", w, i, k, expanded_tau_cd[w * nov * nauxv + i * nauxv + k]);
                }
        }
    }

    // for (int i = 0; i < 200; i++){
    //   printf("expanded_tau_cd[%d]: %f \n", i, expanded_tau_cd[i]);
    // }
    ...
    for (int w = 0; w < naux; ++w) {
        ...
        printf("Done with the elementwise_multiply (u2w = u2 * tau_w )\n");
        // for (int i = 0; i < 200; i++){
        //   printf("u2w[%d]: %f \n", i, u2w[i]);
        // }       
        ...
        dgemm_("T", "N", &nauxv_int, &nauxv_int, &nov_int, &D1,
       u2w, &nov_int, // A (transposed)
       J, &nov_int,   // B (non-transposed)
       &D0, vw, &nauxv_int); // C
        // dgemm_("T", "N", &nauxv_int, &nauxv_int, &nov_int, &D1, J, &nov_int, u2w, &nov_int, &D0, vw, &nauxv_int);
        ...
        //   printf("u2new[%d]: %.15e \n", i, u2new[i]);
        // }
        // dgemm_("T", "T", &nov_int, &nauxv_int, &nauxv_int, &D1, u2w, &nov_int, vw, &nauxv_int, &D0, u2new, &nauxv_int);
